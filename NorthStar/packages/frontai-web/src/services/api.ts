...
import { ApiError, extractErrorMessage, createReadableMessage, getErrorMessage } from '@ns/shared';
...
export const compassApi = {
  ...
  async createComment(id: string, input: CreateContentCommentRequest) {
    return unwrap(await request<ApiEnvelope<{ comment: import('@ns/shared').ContentCommentRecord }>>(`/api/compass/content/${encodeURIComponent(id)}/comments`, {
      method: 'POST',
      body: JSON.stringify(input),
    }));
  },
...
export const complianceApi = {
  ...
  async exportUserData() {
    return unwrap(await request<ApiEnvelope<import('@ns/shared').DataExportResponse>>('/api/compliance/data-export'));
  },

  async requestAccountDeletion(input: { reason?: string }) {
    return unwrap(await request<ApiEnvelope<import('@ns/shared').AccountDeletionRequestRecord>>('/api/compliance/account-deletions', {
      method: 'POST',
      body: JSON.stringify(input),
    }));
  },
